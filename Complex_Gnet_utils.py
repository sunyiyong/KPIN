...
class Gnet(torch.nn.Module):
    def __init__(self, global_vars):
        ...
        self.GRU_in_dim        = global_vars.Gnet_GRU_in_dim
        self.GRU_hn_dim        = global_vars.Gnet_GRU_hn_dim
        self.GRU_num_layers    = 1
        self.GRU_num_direction = 1
        self.GRU_batch_size    = 1
        self.GRU_seq_len       = 1
        self.FC2_out_dim       = global_vars.Gnet_FC2_out_dim
    ...
    def one_ssm_loop(self, yt, global_vars):
        ...
        m1y_re = torch.matmul(global_vars.Q.real, h_prior_re) - torch.matmul(global_vars.Q.imag, h_prior_im)  # (p,n,1), 数量级E-6
        m1y_im = torch.matmul(global_vars.Q.real, h_prior_im) + torch.matmul(global_vars.Q.imag, h_prior_re)  # (p,n,1), 数量级E-6
        self.m1y = torch.complex(m1y_re, m1y_im)  # 复数(n,)，量级e-6
...
def split_dataset_Gnet( global_vars ):
    ...
    train_start  = global_vars.Gnet_pretrain_start
    test_start   = global_vars.Gnet_pretest_start
    num_pretrain = global_vars.Gnet_pretrain_num_all_sub
    len_pretrain = global_vars.Gnet_pretrain_len_sub

    num_pretest  = global_vars.Gnet_num_pretest
    len_pretest  = global_vars.Gnet_pretest_len
    input_dim    = y.shape[0]
    output_dim   = h.shape[0]

    assert input_dim == global_vars.init_tau * global_vars.init_N, "Gnet_GRU_in_dim 「init_y」 does not match the expected value."
