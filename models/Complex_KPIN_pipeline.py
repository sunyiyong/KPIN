...
Gnet_pipeline module: to train and test KPIN.
...
class Gnet_pipeline:
    ...
    def __init__(self, global_vars ):
        ...
        #
        self.num_all_sub             = global_vars.KPIN_train_num_all_sub
        self.pretrain_len_sub        = global_vars.KPIN_train_len_sub
        self.num_pretest             = global_vars.KPIN_num_test
        self.Gnet_pretest_start      = global_vars.KPIN_test_start
        self.pretest_len             = global_vars.KPIN_test_len
        self.Gnet_pretrain_len       = global_vars.KPIN_train_len
        #
        self.pretrain_num_epoch      = global_vars.KPIN_train_num_epoch
        self.pretrain_batchsize      = global_vars.KPIN_train_batchsize
        #
        self.pretrain_learningRate   = global_vars.KPIN_train_learningRate
        self.pretrain_weightDecay    = global_vars.KPIN_train_weightDecay
        #
    ...
    def KPIN_train(self, global_vars):
        ...
        self.num_epochs = global_vars.KPIN_train_num_epoch
        ...
        while True and ti < self.num_epochs:
            ...
            for j in range(0, self.pretrain_batchsize):
                if global_vars.KPIN_train_initSSM_by_KF_posterior:
                    n_e = random.randint(1, self.num_all_sub - 1)
                    last_step = n_e * self.pretrain_len_sub - 1
                    self.Gnet.m1x_prior = global_vars.KFilter.KFilter_prior[last_step,
                                          :].to(self.dev)
                    self.Gnet.m1x_posterior = global_vars.KFilter.KFilter_posterior[
                ...
                self.Gnet.hn = torch.zeros(1, 1, global_vars.KPIN_GRU_hn_dim).to(self.dev)
            ...
            if global_vars.KPIN_supervised_by == "h_prior":
                LOSS_mean = LOSS_h_prior / self.pretrain_batchsize
            elif global_vars.KPIN_supervised_by == "h_posterior":
                LOSS_mean = LOSS_h_posterior / self.pretrain_batchsize
            elif global_vars.KPIN_supervised_by == "y_predict_noisy":
    ...
    def KPIN_test(self, global_vars):
        ...
        for j in range(0, self.num_pretest):
            self.Gnet.m1x_prior = global_vars.KFilter.KFilter_prior[
            ...
            for t in range(0, self.pretest_len):
                ...
                if self.ti == global_vars.KPIN_train_num_epoch - 1:
